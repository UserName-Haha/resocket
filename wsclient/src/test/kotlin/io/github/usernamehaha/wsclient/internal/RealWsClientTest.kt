...
import io.github.usernamehaha.wsclient.DisconnectCause
import io.github.usernamehaha.wsclient.Heartbeat
import io.github.usernamehaha.wsclient.HeartbeatTimeoutException
import io.github.usernamehaha.wsclient.ReconnectPolicy
...
class RealWsClientTest {
    private val transport = FakeTransport()
    private val events = CopyOnWriteArrayList<WsEvent>()
    ...
    private fun TestScope.client(configure: WsConfig.() -> Unit = {}): RealWsClient {
        val config = WsConfig().apply {
    ...
    private fun TestScope.connected(client: RealWsClient): FakeConnection {
        client.connect()
        runCurrent()
    ...
    private inline fun <reified T : WsEvent> events(): List<T> = events.filterIsInstance<T>()
    ...
    fun `连接失败后按退避延迟重连`() = runTest {
        ...
        assertEquals(listOf(1, 2, 3), events<WsEvent.ReconnectScheduled>().map { it.attempt })
        client.close()
    }
    ...
    fun `策略放弃后回到 Disconnected 并带上原因，之后可以再次 connect`() = runTest {
        ...
        assertEquals(1, events<WsEvent.ReconnectAbandoned>().size)
    ...
    fun `策略抛异常时放弃重连并发事件`() = runTest {
        ...
        assertEquals("reconnectPolicy", events<WsEvent.CallbackFailed>().single().name)
        client.close()
    }
    ...
    fun `每次连接都重新取 url，取 url 失败按连接失败处理`() = runTest {
        ...
        transport.latest.fail(IOException())
        advanceTimeBy(1.seconds + 1.milliseconds)
        assertEquals("url", events<WsEvent.CallbackFailed>().single().name)
    ...
    fun `连接之前的订阅在连上后发送，连接之后的订阅立即发送`() = runTest {
        ...
        assertEquals(listOf(false, false), events<WsEvent.Subscribed>().map { it.restored })
        first.cancelAndJoin()
    ...
    fun `同一个 topic 多个收集者只订阅一次，最后一个离开时才退订`() = runTest {
        ...
        assertEquals("trade", events<WsEvent.Unsubscribed>().single().topic)
        client.close()
    }
    ...
    fun `重连后先发 greeting，再按注册顺序恢复仍然有效的订阅`() = runTest {
        ...
        assertEquals(
            ...
            events<WsEvent.Subscribed>().takeLast(2).map { it.topic to it.restored },
        )
    ...
    fun `没有退订消息的订阅离开时什么都不发`() = runTest {
        ...
        assertTrue(events<WsEvent.Unsubscribed>().isEmpty())
        client.close()
    }
    ...
    fun `订阅消息发送失败时发事件，下次重连后恢复`() = runTest {
        ...
        assertEquals(trades.subscribeMessage, events<WsEvent.SendFailed>().single().message)
        ...
        runCurrent()
        assertEquals(listOf(trades.subscribeMessage), transport.latest.sent)
        assertEquals(listOf(false), events<WsEvent.Subscribed>().map { it.restored })
        job.cancelAndJoin()
        client.close()
    }
    ...
    fun `topicOf 抛异常时消息仍然出现在 messages 里`() = runTest {
        ...
        assertEquals("topicOf", events<WsEvent.CallbackFailed>().single().name)
    ...
    fun `dropOldest 保留最新的消息并上报丢弃数`() = runTest {
        ...
        assertEquals(listOf(1L), events<WsEvent.MessagesDropped>().map { it.count })

        // 一秒内的其余丢弃攒到下一次上报
        advanceTimeBy(1.seconds + 1.milliseconds)
        repeat(5) { connection.receive("trade:late$it") }
        runCurrent()
        val dropped = events<WsEvent.MessagesDropped>()
    ...
    fun `收集者离开时缓冲区里剩下的消息不算丢弃`() = runTest {
        ...
        job.cancelAndJoin()
        runCurrent()
        assertTrue(events<WsEvent.MessagesDropped>().isEmpty())
        client.close()
    }
