...
import io.github.usernamehaha.resocket.internal.OkHttpTransport
import io.github.usernamehaha.resocket.internal.RealReSocket
...
public interface ReSocket : Closeable {
    ...
    /**
     * 收到的全部文本消息，包括不属于任何订阅的。每个收集者有独立的缓冲区，使用 [ReSocketConfig.backpressure]。
    ...
    public companion object {
         ...
         */
        public fun create(client: OkHttpClient = OkHttpClient(), configure: ReSocketConfig.() -> Unit): ReSocket {
            val config = ReSocketConfig().apply(configure)
            requireNotNull(config.urlProvider) { "必须通过 url(...) 配置连接地址" }
            return RealReSocket(config, OkHttpTransport(client, config), TimeSource.Monotonic)
        }
    }
}
