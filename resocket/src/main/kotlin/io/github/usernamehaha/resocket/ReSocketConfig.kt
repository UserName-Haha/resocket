...
/** [ReSocket.create] 的配置。除了 url 以外都有默认值。 */
public class ReSocketConfig internal constructor() {
     ...
     * （心跳应答、订阅确认等），它只会出现在 [ReSocket.messages] 里。
     *
     * 每条消息调用一次，在 OkHttp 的读线程上执行，应当尽量轻。不设置的话 [ReSocket.subscribe] 收不到任何消息。
     */
    ...
    /** [ReSocket.subscribe] 和 [ReSocket.messages] 的默认背压策略。 */
    public var backpressure: Backpressure = Backpressure.dropOldest()

    internal var listener: ReSocketListener? = null
     ...
     */
    public fun listener(listener: ReSocketListener) {
